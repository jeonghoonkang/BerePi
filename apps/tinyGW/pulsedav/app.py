...
def main() -> None:
    ...
    with col1:
        ...
        webdav_sub = st.text_area("WebDAV 서브 디렉토리 (선택, 여러 줄 가능)", format_webdav_sub(settings["webdav"].get("sub", "")), height=80)
    ...
    updated_settings = {
        "webdav": {
            ...
            "sub": parse_webdav_sub_input(webdav_sub),
                ...
                st.success(
                    "\n".join(
                        [
                            ...
                            f"- 업로드 개수: {len(result.get('remote_paths', [result['remote_path']]))}",
                        ]
                    )
                )
    ...
    st.code(
        "\n".join(
            [
                '@reboot cd /Users/tinyos/devel_opment/BerePi/apps/tinyGW/pulsedav && { /usr/bin/python3 -c \'from datetime import datetime; from time_utils import SEOUL; d=datetime.now(SEOUL); print(f"{d.year:04d}-{d.month:02d}-{d.day:02d} {d.hour:02d}:{d.minute:02d}:{d.second:02d} {d.tzname()}")\'; /usr/bin/python3 sender.py --once; } > pulsedav.log 2>&1',
                f'*/{max(1, min(int(interval_minutes), 59))} * * * * cd /Users/tinyos/devel_opment/BerePi/apps/tinyGW/pulsedav && {{ /usr/bin/python3 -c \'from datetime import datetime; from time_utils import SEOUL; d=datetime.now(SEOUL); print(f"{{d.year:04d}}-{{d.month:02d}}-{{d.day:02d}} {{d.hour:02d}}:{{d.minute:02d}}:{{d.second:02d}} {{d.tzname()}}")\'; /usr/bin/python3 sender.py --once; }} > pulsedav.log 2>&1',
            ]
        ),
        language="cron",
    )
