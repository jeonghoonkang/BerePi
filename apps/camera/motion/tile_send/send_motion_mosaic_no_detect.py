#!/usr/bin/env python3
"""Generate mosaics from motion images and send them via telegram.

This variant of ``send_motion_mosaic`` intentionally skips running the
``detect_people`` routine so that it can operate on systems where CPU usage is
at a premium. It still collects the thirty-two most recent images in
``/var/lib/motion`` (or all images for a specific day when ``--date`` is
provided), assembles mosaics, records disk information to InfluxDB, generates
graphs, and sends the artefacts via ``telegram-send`` with progress reporting
from ``rich``.
"""
...
def main() -> None:
    ...
    if args.pcount:
        ...
        if not images:
            ...
            console.print(
                "Person detection is disabled; skipping --pcount processing."
            )
    ...
    if args.date:
        ...
        console.print("Skipping person detection to reduce CPU usage.")
