...
from client_service import OUTPUT_DIR, underline_text, write_book_pdf
...
def main() -> int:
    ...
    if not markdown_path.exists():
        print(underline_text(f"Markdown file not found: {markdown_path}"), file=sys.stderr)
        return 2
    if markdown_path.suffix.lower() not in {".md", ".markdown"}:
        print(underline_text(f"Input is not a markdown file: {markdown_path}"), file=sys.stderr)
        return 2
    ...
    print(underline_text(f"PDF generation failed: {message}"), file=sys.stderr)
    return 1
