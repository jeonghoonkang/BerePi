function offlineCounterFetch() { return Promise.resolve({ ok: true, json: async () => ({ value: 0 }) }); }
...
function initDownloadCounters() {
    ...
    keys.forEach(key => {
        ...
        offlineCounterFetch(`${counterBaseUrl}/get/${encodeURIComponent(key)}`)
            .then(response => {
    ...
    downloadLinks.forEach(link => {
        link.addEventListener('click', () => {
            ...
            offlineCounterFetch(`${counterBaseUrl}/hit/${encodeURIComponent(key)}`, {
                method: 'GET',
                cache: 'no-store',
                keepalive: true
            })
