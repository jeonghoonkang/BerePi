...
fn runNull(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var runtime = zero_native.Runtime.init(.{
        ...
        .trace_sink = StdoutTraceSink{}.sink(),
        .log_path = null,
...
fn runMacos(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var mac_platform = try zero_native.platform.macos.MacPlatform.initWithOptions(zero_native.geometry.SizeF.init(1440, 940), webEngine(), app_info);
...
fn runLinux(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var linux_platform = try zero_native.platform.linux.LinuxPlatform.initWithOptions(zero_native.geometry.SizeF.init(1440, 940), webEngine(), app_info);
...
fn runWindows(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var windows_platform = try zero_native.platform.windows.WindowsPlatform.initWithOptions(zero_native.geometry.SizeF.init(1440, 940), webEngine(), app_info);
