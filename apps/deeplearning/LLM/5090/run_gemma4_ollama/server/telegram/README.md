...
메시지를 저장하려면 해당 메시지의 **답장**을 선택하고 `/allom`을 전송하세요.
원본 텍스트 또는 사진·파일의 설명(caption)을 줄바꿈과 함께 저장합니다.
답장으로 `/allom 추가 메모`를 보내면 원본과 추가 메모를 구분해서 저장합니다.
일반 `/allom 내용` 사용법은 동일합니다. 텍스트·설명이 없는 첨부는 안내만 하고 저장하지 않습니다.
저장 경로는 명령을 보낸 사용자의 방·토픽·작성자 경로이며, 원본 작성자와 메시지 ID는
Markdown의 `Telegram 원본 출처` 영역에 기록합니다. 사진·파일 자체를 다운로드하지 않습니다.

`/recall "단어"`: 설정된 WebDAV 저장 폴더와 하위 폴더의 모든 Markdown 내용에서
검색어가 포함된 파일을 찾아 목록·링크와 전체 내용을 회신합니다. 대소문자는 구분하지 않으며,
여러 단어는 연속 구문으로 검색합니다. 긴 결과는 여러 메시지로 나누어 전송합니다.
`--webdav_store 1`로 문서 도구가 활성화되어 있어야 합니다.

문서 목록 명령:

- `/list` 또는 `/ls`: 최근 수정된 메모·boost 원본을 합쳐 10개 표시
- `/list all`: 최대 50개 표시. 50개 초과 시 `/list full` 안내
- `/list full`: 전체 항목을 길이에 따라 여러 메시지로 나누어 회신

`/ls all`, `/ls full`도 동일하게 동작합니다. 전체 회신 중 Telegram 전송 제한에
도달하면 지정된 시간만큼 기다렸다가 같은 메시지부터 계속 보냅니다.
