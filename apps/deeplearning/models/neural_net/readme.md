...
# 4. 평가 모드 (Inference)
model.eval()  # 드롭아웃 비활성화 (100% 뉴런 풀가동)
with torch.no_grad():  # 메모리 절약
  preds = model(X.to(device)).argmax(dim=-1)
  acc = (preds.cpu() == y).float().mean() * 100
  print(f"최종 정확도: {acc:.1f}%")
```


---

## 4. 데이터 복잡도 3자 비교 (Sine vs FDC vs LLM)
...
| :--- | :--- | :--- | :--- |
| **입력 차원** | 1차원 연속 실수 ($x \in \mathbb{R}$) | **200차원 다변량 연속 실수** ($x \in \mathbb{R}^{200}$) | 100,000개 이산 토큰의 순서 조합 |
| **경우의 수 크기** | **$1$차원 외길 선** | **연속 공간 $\mathbb{R}^{200}$** (유효 차원: 10~20) | **$100,000^L$** (조합론적 대폭발) |
| **변수 간 관계** | 독립 변수 1개뿐 | **강한 물리/화학적 상호 결합 (공분산)** | **고도의 장기 문맥 의존성** |
| **지배 법칙** | 순수 삼각함수 공식 | 플라즈마/열역학/유체역학 물리 법칙 | 인간의 문법, 상식, 추론 체계 |
| **적정 모델 규모** | **다항 회귀 (파라미터 5개)** | **은닉층 2~4개 딥러닝 or XGBoost** | **수십~수백 층 트랜스포머** (수백억~수천억 파라미터) |

---

## 5. 반도체 FDC 장비 데이터 실무 가이드

> [!TIP]
> 반도체 FDC는 LLM 같은 거대 모델이 전혀 필요하지 않습니다. **데이터의 저장 형태(스냅샷 테이블 vs 원시 파형 시계열)**에 따라 명확히 모델을 선택하십시오.

### 📊 상황 A. 공정 스텝별 '요약 통계량 CSV' 👉 `XGBoost / LightGBM 추천`
* **데이터 형태:** 각 스텝의 센서별 평균(Mean), 표준편차(Std), 최댓값(Max), 최솟값(Min) 테이블
* **장점:**
  * 딥러닝 대비 학습 속도 100배 이상 빠름
  * 노이즈 및 스파이크 이상치에 강인함
  * **설명 가능성(XAI):** SHAP / Feature Importance 분석을 통해 **"200개 센서 중 몇 번 센서가 불량의 원인인가?"**를 공정 엔지니어에게 명확하게 시각화 제공 가능

### 📈 상황 B. 0.1초 단위 '원시 파형 시계열(Raw Trace)' 👉 `경량 딥러닝 추천`
* **데이터 형태:** 웨이퍼 1장 가공 동안 200개 센서가 연속적으로 기록한 수천 줄의 Waveform
* **추천 아키텍처:**
  1. **1D-CNN:** 200개 센서 파형에서 미세 아크(Arcing, 순간 스파크) 초고속 감지
  2. **오토인코더(Autoencoder - 비지도 학습):** 정상 웨이퍼 파형만 학습 후, 이상 발생 시 복원 오차(Reconstruction Error) 급증을 포착 (0.1% 미만의 극단적인 불량 라벨 불균형 극복)
  3. **시계열 트랜스포머 (PatchTST, Informer):** 센서 간 시간적 선후 인과관계를 학습하여 장비 고장 사전 예지보전(PdM)

---

## 6. 소형 모델 최적화를 위한 4대 실전 수칙

1. **너비보다 깊이를 우선하라:** `[1층 × 128뉴런]`보다 `[3층 × 32뉴런]`이 공간을 접는 표현력이 지수적으로 우수합니다.
2. **소형 모델에서는 드롭아웃(Dropout)을 꺼라:** 몇 개 없는 소중한 뉴런을 끄면 치명적인 언더피팅이 발생합니다.
3. **무조건 Adam 옵티마이저를 사용하라:** 파라미터 수가 적은 신경망의 국소 안장점(Saddle Point)을 관성과 적응형 보폭으로 탈출합니다.
4. **입력 특성 표준화(StandardScaler)를 적용하라:** 200개 센서의 단위를 평균 0, 표준편차 1로 맞춰 특정 센서가 뉴런 용량을 독점하지 못하게 방지합니다.
