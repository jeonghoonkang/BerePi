...
1D-CNN: 200개 센서 파형에서 미세 아크(Arcing, 순간 스파크) 초고속 감지

오토인코더(Autoencoder - 비지도 학습): 정상 웨이퍼 파형만 학습 후, 이상 발생 시 복원 오차(Reconstruction Error) 급증을 포착 (0.1% 미만의 극단적인 불량 라벨 불균형 극복)

시계열 트랜스포머 (PatchTST, Informer): 센서 간 시간적 선후 인과관계를 학습하여 장비 고장 사전 예지보전(PdM)

6. 소형 모델 최적화를 위한 4대 실전 수칙
너비보다 깊이를 우선하라: [1층 × 128뉴런]보다 [3층 × 32뉴런]이 공간을 접는 표현력이 지수적으로 우수합니다.

소형 모델에서는 드롭아웃(Dropout)을 꺼라: 몇 개 없는 소중한 뉴런을 끄면 치명적인 언더피팅이 발생합니다.

무조건 Adam 옵티마이저를 사용하라: 파라미터 수가 적은 신경망의 국소 안장점(Saddle Point)을 관성과 적응형 보폭으로 탈출합니다.

입력 특성 표준화(StandardScaler)를 적용하라: 200개 센서의 단위를 평균 0, 표준편차 1로 맞춰 특정 센서가 뉴런 용량을 독점하지 못하게 방지합니다.
