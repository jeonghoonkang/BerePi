...
class SHT25:
    ...
    def __init__(self, device_number=1):
        self.i2c = lgpio.i2c_open( device_number, self._I2C_ADDRESS )
        #fcntl.ioctl(self.i2c, self.I2C_SLAVE, 0x40)
        lgpio.i2c_write_byte(self.i2c, self._SOFTRESET)
        time.sleep(0.050)

    def read_temperature(self):    
        lgpio.i2c_write_byte(self.i2c, self._TRIGGER_TEMPERATURE_NO_HOLD)
        time.sleep(self._TEMPERATURE_WAIT_TIME)
        count, data = lgpio.i2c_read_device(self.i2c,2)
        #if self._calculate_checksum(data, 1) == (data[1]):
...
if __name__ == "__main__":
    try:
        while True:
            with SHT25(1) as sht25:
                print ("Temperature: %d" % sht25.read_temperature())
